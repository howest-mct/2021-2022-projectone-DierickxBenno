...
const setColor = function () {
  ...
  document.querySelector(".c-curr_color").style.background = `hsl(${Math.round(
    (slider.value / 255) * 360
  )}, 100%, 50%)`;
  slider.addEventListener("change", function () {
    document.querySelector(
      ".c-curr_color"
    ).style.background = `hsl(${Math.round(
      (slider.value / 255) * 360
    )}, 100%, 50%)`;
    socketio.emit("F2B_set_color", { hue: slider.value });
    // console.log("new slider value sent")
...
const listenToPresets = function () {
  ...
  for (const color of presets) {
    color.addEventListener("click", function () {
      ...
      switch (kleurId) {
        case "0":
          document.querySelector(".c-curr_color").style.background =
            "linear-gradient(to bottom right, #FF0018, #FFA52C, #FFFF41, #008018, #0000F9, #86007D)";
          socketio.emit("F2B_set_color", { hue: "999" });
          break;
        case "1":
          document.querySelector(".c-curr_color").style.background = "white";
          socketio.emit("F2B_set_color", { hue: "998" });
          // console.log("white!!")
          break;
      }
    });
...
const listenToInterval = function () {
  ...
  for (const knop of knoppen) {
    knop.addEventListener("click", function () {
      ...
      for (const knop of knoppen) {
        knop.classList.remove("c-current-interval");
      }
      switch (interval) {
        case "1d":
          handleHistoryDay();
          knop.classList.add("c-current-interval");
          break;
  ...
  socketio.on("B2F_curr_hue", function (jsonObject) {
    ...
    if (presets.includes(jsonObject.hue)) {
      ...
      document.querySelector(
        ...
        (document.querySelector(".c-slider").value / 255) * 360
      )}, 100%, 50%)`;
    }
  });
...
const show_graph_temp = function () {
  //temperature
  ...
  chart_temp = new ApexCharts(
    document.querySelector(".g-temperature"),
    options_temp
  );
};
...
const show_graph_steps = function () {
  ...
  chart_steps = new ApexCharts(
    document.querySelector(".g-steps"),
    options_steps
  );
};
